...
const replicaUrls = process.env.DATABASE_URL_REPLICAS
  ? process.env.DATABASE_URL_REPLICAS.split(',').map(url => url.trim()).filter(Boolean)
  : [];
...
export async function connectPrisma() {
  try {
    ...
    console.log(`🐘 Database connected successfully${replicaClients.length > 0 ? ` (with ${replicaClients.length} read replicas)` : ''}`);
  } catch (error) {
