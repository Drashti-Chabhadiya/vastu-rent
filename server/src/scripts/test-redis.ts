...
async function runTest() {
  ...
  if (!status) {
    console.error(
      '❌ Redis is not connected! Make sure your local Redis server is running.',
    )
    process.exit(1)
  }
  ...
  const testData = {
    greeting: 'Hello from Antigravity Redis Cacher!',
    timestamp: Date.now(),
  }
  ...
  if (val1 === null && val2 === null && val3 === null) {
    ...
    console.error('❌ cacheDelPattern verification failed!', {
      val1,
      val2,
      val3,
    })
