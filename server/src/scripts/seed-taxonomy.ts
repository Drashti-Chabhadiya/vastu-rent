...
const defaultCategories = [
  ...
  },
  {
    ...
    icon: 'Sofa',
    color: 'var(--color-category-amber)',
  },
  {
    name: 'Computers & Laptops',
    icon: 'Laptop',
    color: 'var(--color-category-blue-dark)',
  },
  {
  ...
  },
  {
    ...
    icon: 'Flower2',
    color: 'var(--color-category-lime)',
  },
  {
    name: 'Apparel & Designer Wear',
    icon: 'Shirt',
  ...
  {
    name: 'Sports & Rideables',
    icon: 'Bike',
    color: 'var(--color-category-red)',
  },
  {
    name: 'Kitchen & Dining Appliances',
    icon: 'Utensils',
    color: 'var(--color-category-gray)',
  },
]
...
async function main() {
  ...
  if (fs.existsSync(taxonomyPath)) {
    try {
      console.log(
        `Found taxonomy file at: ${taxonomyPath}. Reading contents...`,
      )
      ...
      if (Array.isArray(parsed)) {
        console.log(
          `Successfully parsed taxonomy array with ${parsed.length} items.`,
        )
      ...
      console.error(
        'Failed to parse _taxonomy.json, falling back to default categories.',
        error,
      )
    }
  } else {
    console.log(
      `Taxonomy file not found at: ${taxonomyPath}. Using default premium category list.`,
    )
  }

  console.log(
    `Seeding ${categoriesToSeed.length} categories to the database...`,
  )
  ...
  for (const category of categoriesToSeed) {
    try {
      ...
      console.log(
        `Upserted category: "${result.name}" (ID: ${result.id}, Icon: ${result.icon}, Color: ${result.color})`,
      )
      successCount++
    } catch (err) {
  ...
  console.log(
    `Seeding complete. Successfully seeded ${successCount} out of ${categoriesToSeed.length} categories.`,
  )
}
