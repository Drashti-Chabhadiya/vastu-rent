...
export class PaymentService {
  ...
  async createBookingSession(userId: string, rentalId: string) {
    ...
    if (isMock) {
      console.log(
        `ℹ️  Stripe key is a placeholder. Generating a simulated Stripe Checkout session for booking.`,
      )
    ...
    try {
      ...
      console.error(
        '❌ Stripe Booking Checkout Session Creation failed, falling back to mock session:',
        error,
      )
      const mockSessionId = `mock_booking_session_${rentalId}_${Date.now()}`
      const mockSessionUrl = `${clientUrl}/account/bookings?session_id=${mockSessionId}&rental_id=${rentalId}`
    ...
    try {
      ...
      if (session.payment_status !== 'paid') {
        ...
        const updatedRental = await prisma.$transaction(async (tx) => {
          const rent = await tx.rental.findUnique({
            where: { id: metaRentalId },
          })
      ...
      try {
        ...
        console.error(
          'Failed to cancel rental booking on verification error:',
          cancelErr,
        )
      }
      throw new Error(
        `Failed to verify booking payment session: ${error.message}`,
      )
    }
  }
