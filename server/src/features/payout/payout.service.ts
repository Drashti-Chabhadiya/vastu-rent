...
export class PayoutService {
  async getEarningsDashboard(userId: string) {
    ...
    const withdrawableBalance = Math.max(
      0,
      netEarnings - (pendingPayoutsTotal + completedPayoutsTotal),
    )
    ...
    const productBreakdown = Object.values(productStatsMap).sort(
  ...
  async createPayoutRequest(userId: string, amount: number) {
    ...
    if (amount > dashboard.stats.withdrawableBalance) {
      throw new Error(
        `Insufficient balance! Your maximum withdrawable balance is ₹${dashboard.stats.withdrawableBalance.toLocaleString()}`,
      )
    }
