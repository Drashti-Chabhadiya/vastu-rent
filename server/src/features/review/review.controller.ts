...
export class ReviewController {
  async getAllReviews(request: FastifyRequest, _reply: FastifyReply) {
    ...
    reviews.forEach((r: any) => {
      ...
      if (
        r.product &&
        r.product.user &&
        r.product.user.showProfile === false &&
        r.product.user.id !== currentUserId
      ) {
  ...
  async createReview(request: FastifyRequest, reply: FastifyReply) {
    ...
    try {
      ...
      return reply
        .status(400)
        .send({ message: error.message || 'Failed to submit review' })
    }
  }
  ...
  async replyToReview(request: FastifyRequest, reply: FastifyReply) {
    ...
    try {
      ...
      return reply
        .status(400)
        .send({ message: error.message || 'Failed to submit reply' })
    }
  }
}
