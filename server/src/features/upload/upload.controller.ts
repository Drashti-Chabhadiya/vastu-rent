...
export class UploadController {
  async uploadProfileImage(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      const { url } = await cloudinaryService.uploadImage(base64, 'profiles', userId);
      ...
      return reply.status(400).send({ message: error.message || "Profile upload failed" });
    }
  }
  ...
  async uploadProductImage(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      // Upload using user's custom credentials
      const { url } = await cloudinaryService.uploadImage(base64, 'products', userId);
      return { url };
    } catch (error: any) {
      console.error('Product Image Upload Error:', error);
      return reply.status(400).send({ message: error.message || "Product image upload failed" });
    }
  }
}
