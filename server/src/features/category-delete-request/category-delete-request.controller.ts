...
export class CategoryDeleteRequestController {
  ...
  async getAllRequests(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      const requests = await categoryDeleteRequestService.getAllRequests(
        userId || '',
        role || 'user',
      )
      return { requests }
  ...
  async processRequest(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      const deleteRequest =
        await categoryDeleteRequestService.updateRequestStatus(
          id,
          status,
          session.user.id,
        )
...
export const categoryDeleteRequestController =
  new CategoryDeleteRequestController()
