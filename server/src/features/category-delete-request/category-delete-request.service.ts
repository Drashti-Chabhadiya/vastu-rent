...
export class CategoryDeleteRequestService {
  async createRequest(categoryId: string, userId: string, reason?: string) {
    ...
    if (category.userId !== userId) {
      ...
      if (user?.role !== 'admin') {
        throw new Error(
          'Forbidden: You can only delete categories you proposed',
        )
      }
    }
    ...
    if (existingRequest)
      throw new Error('A deletion request is already pending for this category')
    ...
    if (activeApprovedRequest && activeApprovedRequest.approvedAt) {
      ...
      if (activeApprovedRequest.approvedAt >= oneDayAgo) {
        throw new Error(
          'A deletion request for this category is already approved and active. Please delete it in the pipeline.',
        )
      }
    }
  ...
  async updateRequestStatus(
    ...
    if (status === 'approved') {
      if (!request.categoryId) {
        throw new Error(
          'Category is already deleted or not linked to this request',
        )
      }
