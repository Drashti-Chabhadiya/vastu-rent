...
export class BillingService {
  async createCheckoutSession(userId: string, planName: string, interval: string) {
    ...
    if (isMock) {
      console.log(`ℹ️  Stripe key is a placeholder. Generating a simulated/mock Stripe Checkout session.`);
    ...
    try {
      ...
      console.error("❌ Stripe Checkout Session Creation failed, falling back to mock session:", error);
