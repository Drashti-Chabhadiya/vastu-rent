...
export class DeleteRequestController {
  async createRequest(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      if (user.role !== 'admin') {
        return reply
          .status(403)
          .send({ message: 'Only admins can create deletion requests' })
      }

      const deleteRequest = await deleteRequestService.createRequest(
        productId,
        user.id,
        reason,
      )
  ...
  async processRequest(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      const deleteRequest = await deleteRequestService.updateRequestStatus(
        id,
        status,
        user.id,
      )
