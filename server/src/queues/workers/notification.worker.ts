  ...
  async (job) => {
    ...
    try {
      if (name === JOB_NAMES.NOTIFICATION.PUSH_NOTIFICATION) {
        ...
        console.log(
          `[Notification Worker] FCM Push sent successfully to user ${userId}`,
        )
        ...
        console.log(
          `[Notification Worker] Sending email of type: ${type} to ${emailData.email || 'support'}`,
        )
        ...
        console.log(
          `[Notification Worker] Email type ${type} sent successfully`,
        )
      } else {
      ...
      console.error(
        `❌ [Notification Worker] Error executing job ${name}:`,
        error.message,
      )
      throw error
    }
