  ...
  async (job) => {
    ...
    try {
      if (name === JOB_NAMES.PAYMENT.VERIFY_PAYMENT) {
        ...
        console.log(
          `[Payment Worker] Verification result for rental ${rentalId}:`,
          result,
        )

        if (result.success && result.rental?.paymentStatus === 'paid') {
          // Add generate-invoice job to queue
          await paymentQueue.add(JOB_NAMES.PAYMENT.GENERATE_INVOICE, {
            rentalId,
          })
          console.log(
            `[Payment Worker] Enqueued invoice generation for rental ${rentalId}`,
          )
        }
        ...
        if (!rental) {
          throw new Error(
            `Rental not found for invoice generation: ${rentalId}`,
          )
        }
...
`
        ...
        console.log(
          `[Payment Worker] Invoice email successfully sent to ${rental.renter.email}`,
        )
      } else {
      ...
      console.error(
        `❌ [Payment Worker] Error executing job ${name}:`,
        error.message,
      )
...
paymentWorker.on('failed', (job, err) => {
  console.error(
    `❌ [Payment Worker] Job ${job?.id} failed with error:`,
    err.message,
  )
})
