  ...
  async (job) => {
    ...
    try {
      if (name === JOB_NAMES.IMAGE.OPTIMIZE_IMAGE) {
        ...
        if (!entityId || !entityType || !Array.isArray(imageUrls) || imageUrls.length === 0) {
          throw new Error("Missing parameters for optimize-image job");
        }

        console.log(`[Image Worker] Optimizing ${imageUrls.length} images for ${entityType} ID: ${entityId}`);
        const optimizedUrls: string[] = [];

        for (const imageUrl of imageUrls) {
          try {
            // 1. Download original image
            console.log(`[Image Worker] Downloading original image: ${imageUrl}`);
            ...
            console.log(`[Image Worker] Uploading optimized image to Cloudinary (folder: ${folder})...`);
            const uploadResult = await cloudinaryService.uploadImage(base64, folder);
            ...
            if (oldPublicId) {
              console.log(`[Image Worker] Deleting original high-res image: ${oldPublicId}`);
              await cloudinaryService.deleteImage(oldPublicId);
            }
          } catch (err: any) {
            console.error(`[Image Worker] Failed to optimize image "${imageUrl}":`, err.message);
        ...
        console.log(`[Image Worker] Updating database for ${entityType} ${entityId}...`);
        if (entityType === "user") {
          ...
          console.log(`[Image Worker] User profile image updated successfully to: ${newProfileUrl}`);
          ...
          if (product) {
            ...
            console.warn(`[Image Worker] Product not found during DB update: ${entityId}`);
          }
        }
      } else {
      ...
      console.error(`❌ [Image Worker] Error executing job ${name}:`, error.message);
      throw error;
    }
  },
  {
