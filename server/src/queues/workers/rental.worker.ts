  ...
  async (job) => {
    ...
    try {
      if (name === JOB_NAMES.RENTAL.AUTO_EXPIRY) {
        ...
        console.log(
          `[Rental Worker] Found ${expiredPendingRentals.length} expired pending bookings to cancel.`,
        )
        ...
        for (const rental of expiredPendingRentals) {
          try {
            ...
            console.log(
              `[Rental Worker] Cancelled expired booking ${rental.id} successfully.`,
            )
            ...
            console.error(
              `[Rental Worker] Failed to expire booking ${rental.id}:`,
              err.message,
            )
          }
        }
        ...
        console.log(
          `[Rental Worker] Found ${startingTomorrow.length} rentals starting tomorrow.`,
        )
        ...
        for (const rental of startingTomorrow) {
          try {
            ...
            console.error(
              `[Rental Worker] Failed to send start reminder for rental ${rental.id}:`,
              err.message,
            )
          }
        }
        ...
        console.log(
          `[Rental Worker] Found ${endingTomorrow.length} rentals ending tomorrow.`,
        )
        ...
        for (const rental of endingTomorrow) {
          try {
            ...
            console.error(
              `[Rental Worker] Failed to send return reminder for rental ${rental.id}:`,
              err.message,
            )
          }
        }
        ...
        console.log(
          `[Rental Worker] Found ${oldPendingRentals.length} pending bookings awaiting approval > 12 hours.`,
        )
        ...
        for (const rental of oldPendingRentals) {
          try {
            ...
            console.error(
              `[Rental Worker] Failed to send approval reminder for booking ${rental.id}:`,
              err.message,
            )
          }
        }
        ...
        if (!rental) {
          console.warn(
            `[Rental Worker] Rental ${rentalId} not found for pickup reminder.`,
          )
          return
        }
        ...
        if (rental.status === 'confirmed') {
          console.log(
            `[Rental Worker] Sending pickup reminder for rental ${rental.id} to renter ${rental.renterId}`,
          )
          ...
          console.log(
            `[Rental Worker] Rental ${rentalId} status is ${rental.status} (not 'confirmed'). Skipping pickup reminder.`,
          )
        }
        ...
        if (!rental) {
          console.warn(
            `[Rental Worker] Rental ${rentalId} not found for return reminder.`,
          )
          return
        }
        ...
        if (rental.status === 'picked_up') {
          console.log(
            `[Rental Worker] Sending return reminder for rental ${rental.id} to renter ${rental.renterId}`,
          )
          ...
          console.log(
            `[Rental Worker] Rental ${rentalId} status is ${rental.status} (not 'picked_up'). Skipping return reminder.`,
          )
        }
      } else {
      ...
      console.error(
        `❌ [Rental Worker] Error executing job ${name}:`,
        error.message,
      )
      throw error
    }
...
rentalWorker.on('failed', (job, err) => {
  console.error(
    `❌ [Rental Worker] Job ${job?.id} failed with error:`,
    err.message,
  )
})
