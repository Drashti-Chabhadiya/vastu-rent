  ...
  async (job) => {
    ...
    try {
      if (name === JOB_NAMES.CHAT.UNREAD_COUNT) {
        ...
        if (io) {
          console.log(
            `[Chat Worker] Broadcasting unread count (${unreadCount}) to user_${userId}`,
          )
        ...
        if (!message) {
          console.warn(
            `[Chat Worker] Message ${messageId} not found. Skipping notification.`,
          )
          return
        }
        ...
        if (message.isRead) {
          console.log(
            `[Chat Worker] Message ${messageId} is already marked read. Skipping notification.`,
          )
          return
        }
        ...
        const isMuted =
          message.conversation.mutedBy?.includes(recipientId) ?? false
        if (isMuted) {
          console.log(
            `[Chat Worker] Conversation ${message.conversationId} is muted by recipient ${recipientId}. Skipping notification.`,
          )
          return
        }
        ...
        if (io) {
          const socketsInRoom = await io
            ...
            .fetchSockets()
          isRecipientActive = socketsInRoom.some(
        ...
        if (isRecipientActive) {
          console.log(
            `[Chat Worker] Recipient ${recipientId} is currently active in the chat. Skipping background notification.`,
          )
          return
        }
        ...
        console.log(
          `[Chat Worker] Triggering background notification for user ${recipientId}`,
        )
      ...
      console.error(
        `❌ [Chat Worker] Error executing job ${name}:`,
        error.message,
      )
      throw error
    }
...
chatWorker.on('failed', (job, err) => {
  console.error(
    `❌ [Chat Worker] Job ${job?.id} failed with error:`,
    err.message,
  )
})
