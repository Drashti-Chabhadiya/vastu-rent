...
export async function initWorkers() {
  ...
  console.log(
    `[BullMQ] Workers registered and active: ${activeWorkers.map((w) => w.name).join(', ')}`,
  )
  ...
  try {
    ...
    console.log(
      `⏰ [BullMQ] Scheduled repeatable task: '${JOB_NAMES.RENTAL.AUTO_EXPIRY}' (every hour)`,
    )
    ...
    console.log(
      `⏰ [BullMQ] Scheduled repeatable task: '${JOB_NAMES.RENTAL.SEND_REMINDERS}' (every 12 hours)`,
    )
  } catch (err: any) {
    console.error(
      '❌ [BullMQ] Error registering repeatable schedules:',
      err.message,
    )
  }
}
