...
export function Journal() {
  ...
  const displayStories =
    fetchedStories && fetchedStories.length > 0 ? fetchedStories.slice(0, 3) : []

  return (
    <section id="journal" className="bg-surface/70">
      <div className={cn('mx-auto', 'max-w-[1400px]', 'px-6', 'py-24', 'md:px-10', 'md:py-32')}>
        <div className={cn('flex', 'items-end', 'justify-between', 'gap-6')}>
          <div>
            <div className={cn('text-[11px]', 'uppercase', 'tracking-[0.22em]', 'text-primary')}>
              — Journal
            </div>
            <h2 className={cn('mt-4', 'font-display', 'text-[clamp(2.1rem,4vw,3.25rem)]', 'leading-[1.05]', 'tracking-tight', 'text-foreground')}>
              Stories from the catalogue.
            </h2>
          </div>
          <Link
            to="/journal"
            className={cn('hidden', 'items-center', 'gap-2', 'text-[13px]', 'font-medium', 'text-foreground/80', 'underline', 'decoration-border', 'decoration-2', 'underline-offset-[8px]', 'transition-all', 'hover:text-primary', 'hover:decoration-primary', 'md:inline-flex')}
          >
            Read the journal{' '}
            <ArrowUpRight className={cn('h-3.5', 'w-3.5', 'transition-transform', 'group-hover:translate-x-0.5', 'group-hover:-translate-y-0.5')} />
          </Link>
        </div>
