...
export const ProductImageViewer = ({
  ...
  }, [isDragging, scale, isDraggingSwipe, isDraggingSwipeDown, selectedImage, images.length, position, getConstraints])
  ...
  }, [isDragging, isDraggingSwipe, isDraggingSwipeDown, selectedImage, images.length, setSelectedImage, handleClose])
  ...
  const handleTouchMove = useCallback((e: React.TouchEvent) => {
    if (isPinching && e.touches.length === 2) {
      ...
      const newScale = clamp(initialScaleRef.current * (dist / initialPinchDistanceRef.current), 0.6, 4)
      ...
      const relativeX = initialPinchMidpointRef.current.x - rect.left - rect.width / 2
      const relativeY = initialPinchMidpointRef.current.y - rect.top - rect.height / 2
      
      const targetX = (initialPinchOffsetRef.current.x - relativeX) * scaleRatio + relativeX + (midX - initialPinchMidpointRef.current.x)
      const targetY = (initialPinchOffsetRef.current.y - relativeY) * scaleRatio + relativeY + (midY - initialPinchMidpointRef.current.y)
  ...
  }, [isOpen, handleNext, handlePrev, handleZoomIn, handleZoomOut, resetZoom, handleClose])
          ...
          >
            ...
            <div 
              ...
              style={{
                ...
                transition: isDraggingSwipe ? 'none' : 'transform 0.35s cubic-bezier(0.16, 1, 0.3, 1)',
              }}
            >
              {images.map((img, idx) => {
                  ...
                  >
                    ...
                    <div
                      style={isActive ? {
                        ...
                        transition: (isDragging && !isDraggingSwipe && !isDraggingSwipeDown) || isPinching
                          ? 'none'
