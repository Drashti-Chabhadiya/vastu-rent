...
export function AboutPanel() {
  ...
  const handleReportUser = async () => {
    try {
      await reportConversation({ conversationId: activeConversation.id, reason: reportReason })
      toast.warning(`Reported ${activeConversation.otherParticipant.name} for: ${reportReason}`)
    } catch (err) {
  ...
  return (
    <div className="w-[320px] shrink-0 bg-[#fbf9f4] border-l border-slate-200/80 flex flex-col h-full overflow-hidden select-none animate-in slide-in-from-right duration-250">
      {/* Header */}
      <div className="bg-[#0d4d38] px-4 py-5 flex items-center justify-between text-white shrink-0 shadow-sm">
        <h3 className="text-[16px] font-bold text-white leading-none font-display">
          {activeTab === 'info' ? 'Chat Info' : 'My Settings'}
        </h3>
        <Button
          variant="ghost"
          size="icon"
      ...
      {/* Segmented Tab Bar */}
      <div className="px-4 py-2 border-b border-slate-200/30 bg-white shrink-0">
        <div className="flex bg-[#faf9f6] border border-slate-200/40 p-0.5 rounded-xl text-xs font-semibold">
          <button
            onClick={() => setActiveTab('info')}
            className={cn(
              'flex-1 py-1.5 rounded-lg text-center transition-all cursor-pointer text-[11px] font-black tracking-wide',
              activeTab === 'info'
                ? 'bg-[#0d4d38] text-white shadow-sm'
      ...
      {activeTab === 'info' ? (
        <div className="flex-1 overflow-y-auto scrollbar-thin px-4 pb-6 pt-4 flex flex-col gap-4">
        ...
        <div className="bg-white border border-slate-200/80 rounded-2xl p-1 flex flex-col gap-1">
          ...
          >
            ...
            <span>{isBlockedByMe ? 'Unblock' : 'Block'} {activeConversation.otherParticipant.name}</span>
          </button>
        </div>
      ...
      <MySettingsPanel isEmbedded={true} onBackToInfo={() => setActiveTab('info')} />
    )}
      ...
      <ReusableAlertDialog
        ...
        title={isBlockedByMe ? `Unblock ${activeConversation.otherParticipant.name}?` : `Block ${activeConversation.otherParticipant.name}?`}
        description={isBlockedByMe ? `Are you sure you want to unblock ${activeConversation.otherParticipant.name}? You will be able to start sending and receiving messages.` : `Are you sure you want to block ${activeConversation.otherParticipant.name}? You will no longer receive their messages.`}
        ...
        description={
          <div className="flex flex-col gap-3 w-full mt-2">
            <span className="text-slate-500 font-semibold text-[12px] leading-relaxed">
              Report {activeConversation.otherParticipant.name} for spam, abuse, or inappropriate content? This report will be reviewed by administrators.
            </span>
            <Select
              value={reportReason}
            ...
            >
              ...
              <SelectContent className="rounded-xl border border-slate-200 bg-card font-bold text-[12px] text-slate-700">
                <SelectItem value="Spam" className="cursor-pointer rounded-lg hover:bg-slate-50">Spam</SelectItem>
                <SelectItem value="Harassment" className="cursor-pointer rounded-lg hover:bg-slate-50">Harassment / Abuse</SelectItem>
                <SelectItem value="Inappropriate Content" className="cursor-pointer rounded-lg hover:bg-slate-50">Inappropriate Content</SelectItem>
                <SelectItem value="Policy Violation" className="cursor-pointer rounded-lg hover:bg-slate-50">Policy Violation</SelectItem>
                <SelectItem value="Other" className="cursor-pointer rounded-lg hover:bg-slate-50">Other Reason</SelectItem>
              </SelectContent>
            </Select>
          </div>
        }
