...
export function WallpaperSettingsDialog({ open, onOpenChange }: WallpaperSettingsDialogProps) {
  const {
    conversations,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md rounded-3xl p-6 border border-slate-200/80 shadow-2xl bg-card">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          {[
            { id: 'classic', name: 'Classic Sage', preview: 'bg-emerald-50/80 border-emerald-200' },
            { id: 'dawn', name: 'Dawn Peach', preview: 'bg-orange-50/80 border-orange-200' },
            { id: 'forest', name: 'Forest Green', preview: 'bg-emerald-100/80 border-emerald-300' },
            { id: 'minimal', name: 'Minimal Slate', preview: 'bg-slate-50/80 border-slate-200' },
          ].map((theme) => {
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive ? "border-emerald-600 bg-emerald-50/20" : "border-slate-200 hover:bg-slate-50"
                )}
              >
