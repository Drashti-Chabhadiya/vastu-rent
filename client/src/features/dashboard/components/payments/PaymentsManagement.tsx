...
export const PaymentsManagement = () => {
  ...
  const [adminActionType, setAdminActionType] = useState<'approved' | 'rejected' | 'paid' | null>(null);
  ...
  const handleRequestPayout = (e: React.FormEvent) => {
    ...
    if (amt > maxWithdrawable) {
      toast.error(`Insufficient balance! Your maximum withdrawable balance is ₹${maxWithdrawable.toLocaleString()}`);
      return;
    }
  ...
  const handleAdminAction = () => {
    ...
    updatePayoutStatus.mutate(
      ...
      },
      {
        ...
        onError: (err: any) => {
          toast.error(err.response?.data?.message || err.message || "Failed to update payout request");
        }
      }
    );
  };
  ...
  const uniqueProducts = Array.from(new Set(recentTransactions.map((t: any) => t.product?.title).filter(Boolean))) as string[];
  ...
  const getPayoutStatusBadge = (status: string) => {
    switch (status) {
      case 'paid':
        return <Badge className="bg-emerald-50 text-emerald-600 border-none px-2.5 py-0.5 rounded-md font-black flex items-center gap-1.5"><CheckCircle2 size={10} /> Paid</Badge>;
      case 'approved':
        return <Badge className="bg-indigo-50 text-indigo-600 border-none px-2.5 py-0.5 rounded-md font-black flex items-center gap-1.5"><Zap size={10} /> Approved</Badge>;
      case 'rejected':
        return <Badge className="bg-red-50 text-red-600 border-none px-2.5 py-0.5 rounded-md font-black flex items-center gap-1.5"><XCircle size={10} /> Rejected</Badge>;
      default:
        return <Badge className="bg-amber-50 text-amber-600 border-none px-2.5 py-0.5 rounded-md font-black flex items-center gap-1.5"><Clock size={10} /> Pending</Badge>;
    }
  };
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          <div className="flex items-center gap-1.5 text-[10px] font-black text-slate-400 uppercase tracking-widest">
            ...
            <span className="text-emerald-600 font-extrabold">Earnings & Payouts</span>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.02)] flex items-center gap-4">
          ...
          <div className="space-y-0.5">
            ...
            <h3 className="text-xl font-black text-slate-800">₹{stats.totalEarnings.toLocaleString()}</h3>
            <span className="text-[9px] font-bold text-slate-400">All successful orders</span>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.02)] flex items-center gap-4">
          ...
          <div className="space-y-0.5">
            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Monthly Revenue</span>
            <h3 className="text-xl font-black text-slate-800">₹{stats.monthlyEarnings.toLocaleString()}</h3>
            <span className="text-[9px] font-bold text-blue-500 font-medium">This current month</span>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.02)] flex items-center gap-4">
          ...
          <div className="space-y-0.5">
            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Commission (10%)</span>
            <h3 className="text-xl font-black text-slate-800">₹{stats.platformCommission.toLocaleString()}</h3>
            <span className="text-[9px] font-bold text-amber-500">Platform charge</span>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.02)] flex items-center gap-4">
          ...
          <div className="space-y-0.5">
            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Withdrawn Paid</span>
            <h3 className="text-xl font-black text-slate-800">₹{stats.completedPayouts.toLocaleString()}</h3>
            <span className="text-[9px] font-bold text-indigo-500">Successfully settled</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-900 p-8 rounded-[2.5rem] text-white flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-xl relative overflow-hidden">
        ...
        <div className="space-y-2 relative z-10">
          ...
          <h3 className="text-sm font-semibold text-slate-400">Available Withdrawable Balance</h3>
          <div className="flex items-baseline gap-1">
            <span className="text-3xl font-black">₹{stats.withdrawableBalance.toLocaleString()}</span>
            <span className="text-[10px] text-slate-400 font-bold">Net Earnings - Pending payouts</span>
          </div>
        </div>
      ...
      {isAdmin && allAdminPayouts && allAdminPayouts.length > 0 && (
        <div className="bg-[#faf7f0] border-2 border-emerald-600/20 p-8 rounded-[2.5rem] shadow-sm space-y-6">
          <div>
            ...
            <p className="text-[11px] font-semibold text-slate-500">Review, approve or reject payout requests from listing listers.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {allAdminPayouts.filter((p: any) => p.status === 'pending' || p.status === 'approved').map((payout: any) => (
              <div key={payout.id} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-between gap-4">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-slate-100 overflow-hidden flex-shrink-0">
                      {payout.owner?.image ? (
                        <img src={payout.owner.image} alt="" className="w-full h-full object-cover" />
                      ) : (
                    ...
                    <div className="space-y-0.5">
                      <h4 className="text-xs font-black text-slate-800">{payout.owner?.name}</h4>
                      <p className="text-[10px] text-slate-400 font-bold truncate">{payout.owner?.email}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <span className="text-xs font-black text-[#1e293b] block">₹{payout.amount.toLocaleString()}</span>
                    <span className="text-[8px] font-bold text-slate-400 block">{format(new Date(payout.createdAt), 'dd MMM yyyy')}</span>
                  </div>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
            <div className="flex flex-wrap gap-4 items-center justify-between">
              <div>
                <h3 className="text-[15px] font-black text-slate-800">Recent Revenue Transactions</h3>
                <p className="text-[11px] font-bold text-slate-400 mt-0.5">Successful orders received from customers.</p>
              </div>
            ...
            <div className="space-y-4">
              {filteredTransactions.length === 0 ? (
                <div className="text-center py-10 bg-slate-50 rounded-2xl border border-slate-100">
                  <span className="text-xs font-black text-slate-400 uppercase tracking-widest">No successful orders found</span>
                </div>
              ) : filteredTransactions.slice(0, 5).map((trans: any) => (
                <div key={trans.id} className="flex items-center justify-between p-4 rounded-2xl border border-slate-50 hover:bg-slate-50/50 transition-all group">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-xl overflow-hidden border border-slate-100 shadow-sm flex-shrink-0">
                      {trans.product?.image ? (
                        <img src={trans.product.image} className="w-full h-full object-cover" />
                      ) : (
                    ...
                    <div className="space-y-0.5">
                      <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">ID #ORD-{trans.id.slice(-5).toUpperCase()}</p>
                      <p className="text-xs font-black text-slate-800 leading-snug">{trans.product?.title}</p>
                      <p className="text-[9px] font-bold text-slate-400">{format(new Date(trans.createdAt), 'dd MMM yyyy, hh:mm a')}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      <span className="text-xs font-black text-slate-800">₹{trans.totalPrice.toLocaleString()}</span>
                      <span className="text-[8px] font-bold text-slate-400 block">Gross</span>
                    </div>
          ...
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
            <div>
              <h3 className="text-[15px] font-black text-slate-800">Earnings Breakdown by Product</h3>
              <p className="text-[11px] font-bold text-slate-400 mt-0.5">Total revenue generated per listing.</p>
            </div>
            <div className="space-y-4">
              {productBreakdown.length === 0 ? (
                <div className="text-center py-10 bg-slate-50 rounded-2xl border border-slate-100">
                  <span className="text-xs font-black text-slate-400 uppercase tracking-widest">No listings product earnings found</span>
                </div>
              ) : productBreakdown.map((item: any) => (
                <div key={item.id} className="flex items-center justify-between p-4 rounded-2xl border border-slate-50">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="space-y-0.5">
                      <h4 className="text-xs font-black text-slate-800 truncate max-w-xs">{item.title}</h4>
                      <p className="text-[9px] font-bold text-slate-400">{item.bookingCount} successful rentals</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <span className="text-xs font-black text-emerald-600">₹{item.totalEarned.toLocaleString()}</span>
                    <span className="text-[8px] font-bold text-slate-400 block">Total Earnings</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
            <div>
              <h3 className="text-[15px] font-black text-slate-800">Payout Settlement History</h3>
              <p className="text-[11px] font-bold text-slate-400 mt-0.5">Track your payout withdrawal request updates.</p>
            </div>
            ...
            <div className="space-y-4">
              {payoutRequests.length === 0 ? (
                <div className="text-center py-10 bg-slate-50 rounded-2xl border border-slate-100 flex flex-col items-center gap-2">
                  ...
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">No payout requests placed</span>
                </div>
              ) : payoutRequests.map((req: any) => (
                <div key={req.id} className="p-4 rounded-2xl border border-slate-50 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-black text-slate-800">₹{req.amount.toLocaleString()}</span>
                    <div>{getPayoutStatusBadge(req.status)}</div>
                  </div>
                  <div className="flex items-center justify-between text-[9px] font-bold text-slate-400">
                    <span>{format(new Date(req.createdAt), 'dd MMM yyyy, hh:mm a')}</span>
                    {req.notes && (
                      <span className="text-red-500 max-w-[150px] truncate" title={req.notes}>Notes: {req.notes}</span>
                    )}
                  </div>
                </div>
              ))}
          ...
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-4">
            ...
            <p className="text-[11px] font-semibold text-slate-500 leading-relaxed">
              Platform settlements are subject to a standard 10% commission. Payout requests are verified by auditing dispute histories and cleared within 24-48 hours.
            </p>
            <div className="border-t border-slate-50 pt-4 flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest">
              <Lock size={12} className="text-emerald-600" /> BANK-LEVEL SSL ENCRYPTED
            </div>
          </div>
        </div>
      </div>
        ...
        <DialogContent className="max-w-md p-8 border-none bg-white rounded-[2.5rem] shadow-2xl font-sans">
          <form onSubmit={handleRequestPayout} className="space-y-6">
            <div className="space-y-2.5">
              ...
              <h3 className="text-xl font-extrabold text-slate-800">Request Payout</h3>
              <p className="text-[11px] font-bold text-slate-400">
                Amount will be reviewed by admin and settled directly to your registered bank account/UPI within 24-48 hours.
              </p>
            </div>

            {/* Input field */}
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Withdrawal Amount (₹)</label>
            ...
            <div className="bg-amber-50/50 p-4 rounded-2xl border border-amber-100/50 flex items-start gap-2">
              ...
              <p className="text-[10px] font-semibold text-amber-800 leading-relaxed">
                By requesting this withdrawal, you authorize standard 10% platform fee calculations against your gross earnings bookings database.
              </p>
            </div>
      ...
      <Dialog open={!!selectedAdminPayout} onOpenChange={(open) => !open && setSelectedAdminPayout(null)}>
        {selectedAdminPayout && (
          <DialogContent className="max-w-md p-8 border-none bg-white rounded-[2.5rem] shadow-2xl font-sans">
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                <h3 className="text-xl font-extrabold text-slate-800">
                  {adminActionType === 'rejected' ? 'Reject Payout Request' : 'Approve & Mark as Paid'}
                </h3>
                <p className="text-[11px] font-bold text-slate-400">
                  Request by <strong>{selectedAdminPayout.owner?.name}</strong> for <strong>₹{selectedAdminPayout.amount.toLocaleString()}</strong>.
                </p>
              </div>

              {/* Action Notes Input */}
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Notes / Reason / Transaction ID</label>
                <textarea 
                  placeholder={adminActionType === 'rejected' ? "Enter rejection reason..." : "Enter transaction ID or payment notes..."}
                  value={adminNotes}
              ...
              <div className="flex gap-4">
                ...
                >
                  {updatePayoutStatus.isPending ? 'Updating...' : adminActionType === 'rejected' ? 'Confirm Reject' : 'Confirm Pay'}
                </Button>
              </div>
            </div>
          </DialogContent>
