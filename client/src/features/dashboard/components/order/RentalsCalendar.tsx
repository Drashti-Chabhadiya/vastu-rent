...
export const RentalsCalendar = () => {
  ...
  const filteredOrders = (orders || []).filter((order: any) => {
    const matchProduct = selectedProduct === 'all' || order.product?.title === selectedProduct;
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'confirmed':
      case 'active':
        return <Badge className="bg-emerald-50 text-emerald-600 border-none px-2.5 py-0.5 rounded-md font-black flex items-center gap-1.5"><CheckCircle2 size={10} /> Confirmed</Badge>;
      case 'completed':
        return <Badge className="bg-blue-50 text-blue-600 border-none px-2.5 py-0.5 rounded-md font-black flex items-center gap-1.5"><CheckCircle2 size={10} /> Completed</Badge>;
      case 'rejected':
      case 'cancelled':
        return <Badge className="bg-red-50 text-red-600 border-none px-2.5 py-0.5 rounded-md font-black flex items-center gap-1.5"><XCircle size={10} /> Cancelled</Badge>;
      default:
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-dash-text-soft text-sm font-medium">
            Track product reserved schedules, monitor date occupancy, and prevent double bookings.
          </p>
        </div>
            ...
            return (
              <div 
                ...
                className={cn(
                  ...
                  isToday && "bg-[#faf7f0]/40 border-l-2 border-l-[#059669] lg:border-l-0"
                )}
              >
                ...
                <div className="flex items-center justify-between">
                  <span className={cn(
                    ...
                    isToday ? "bg-[#059669] text-white shadow-sm" : isSelectedMonth ? "text-slate-800" : "text-slate-400"
                  )}>
                    {format(day, 'd')}
                  </span>
                  ...
                  {bookings.length > 0 && (
                    <span className="text-[8px] font-black uppercase text-slate-400 shrink-0 bg-slate-100 px-1.5 py-0.5 rounded">
                      {bookings.length} {bookings.length === 1 ? 'Book' : 'Books'}
                    </span>
                  )}
                </div>
      ...
      <div className="bg-slate-50 p-5 rounded-[2rem] border border-slate-100 flex items-start gap-4">
        ...
        <div className="space-y-1">
          <h4 className="text-sm font-black text-slate-800 uppercase tracking-wider">Reserved occupancy validation</h4>
          <p className="text-xs font-semibold text-slate-500 leading-relaxed max-w-2xl">
            Confirming a Cash or razorpay booking automatically blocks the specific listing dates from being reserved by other renters. Rejected/Cancelled requests immediately free the dates.
          </p>
        </div>
      </div>
            ...
            <div className="p-8 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white p-4 rounded-2xl border border-slate-100 space-y-1">
                  <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block">Customer Info</span>
                  <div className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                    <UserIcon size={12} className="text-slate-400" />
                    <span>{selectedOrder.renter?.name}</span>
                  </div>
                  <span className="text-[10px] font-medium text-slate-400 block truncate">{selectedOrder.renter?.email}</span>
                </div>

                <div className="bg-white p-4 rounded-2xl border border-slate-100 space-y-1">
                  <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block">Status & Method</span>
                  <div className="flex gap-1.5 items-center">{getStatusBadge(selectedOrder.status)}</div>
                  <span className="text-[10px] font-black uppercase text-slate-400 block tracking-tight pt-1">
                    Via {selectedOrder.paymentMethod === 'cash' ? 'Cash / CoD' : 'Online'}
                  </span>
                </div>
              </div>
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-100 space-y-3">
                <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block">Reserved Duration</span>
                <div className="flex items-center justify-between text-xs font-bold text-slate-700">
                  <div className="flex flex-col">
                    <span className="text-[9px] text-slate-400 uppercase">From</span>
                    <span>{format(new Date(selectedOrder.startDate), 'dd MMM yyyy')}</span>
                  </div>
                  <ChevronRight size={14} className="text-slate-300 mt-2" />
                  <div className="flex flex-col text-right">
                    <span className="text-[9px] text-slate-400 uppercase">To</span>
                    <span>{format(new Date(selectedOrder.endDate), 'dd MMM yyyy')}</span>
                  </div>
                </div>
                <div className="border-t border-slate-100 pt-2 flex items-center justify-between text-[11px] font-bold text-slate-500">
                  ...
                  <span className="text-[#059669] font-black">{calculateDuration(selectedOrder.startDate, selectedOrder.endDate)} Days</span>
                </div>
              </div>
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-100 space-y-2 text-xs font-semibold text-slate-500">
                <div className="flex justify-between">
                  ...
                  <span className="text-slate-800 font-bold">₹{(selectedOrder.product?.price || selectedOrder.totalPrice).toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span>Security deposit</span>
                  <span className="text-slate-800 font-bold">₹{(selectedOrder.depositAmount || 0).toLocaleString()}</span>
                </div>
              ...
              {selectedOrder.status === 'pending' ? (
                ...
                <div className="text-center py-4 bg-slate-100/50 rounded-2xl border border-slate-100">
                  <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Request already processed</span>
                </div>
              )}
            </div>
          </DialogContent>
